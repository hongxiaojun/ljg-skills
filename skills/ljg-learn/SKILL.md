...
**格式规则（零例外）：**
- 输出必须是纯 markdown 语法
- 使用 denote 风格的文件名：`{timestamp}--概念解剖-{概念名}__concept.md`
- 标题用 `#` `##` `###` 层级
- 加粗用 `**bold**`
- 代码用反引号 `` `code` `` 或代码块
- 列表用 `- item` 或 `1. item`
...
### 结构图
纯 ASCII 画出概念的骨架（只用 +-|/\<>*=_.,:;!'" 等基本符号，不用 Unicode 绘图字符）
```
